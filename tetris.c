/* Main source file for the Tetris game on Atari ST */
...
int main() {
    ...
    /* Swap buffers after first frame*/
    ...
    while (quit != 1) {
        ...
        /* VBL ISR sets render_request once per frame (70hz) */
        if (render_request) {

            /* Get old piece for this buffer */
            if (first_render) {
            ...
            if (game_model.redraw_matrix) {
                ...
                UINT32 *front_ptr;  
                int row, word;
                int bytes_per_row = 80;
                int matrix_pixel_rows = MATRIX_ROWS * CELL_SIZE;
                int words_per_matrix_row = (MATRIX_COLS * CELL_SIZE) / 32;
